...
```
## **Работа с удаленными репозоториями**
>Прежде всего нужно авторизоваться на [GitHub](https://github.com).
### **Создание FORK`а существующего репозитория**
Зайти на какой либо проект.

>Например: [DZ_rep3](https://github.com/CHRTla/DZ_rep3)

Нажать кнопку `Fork` справа вверху экрана.\
Тем самым создастся привязанная к текущему проекту полная копия основной ветки (main/master) в вашем репозитории.
### **Клонирование FORK`a на свой компьютер**
Создаем папку в необходимом нам месте и заходим в нее через терминал.\
Через браузер заходим в свой репозиторий. Находим только что `Fork`нутый проект и заходим в него.\
Нажимаем зеленую кнопку `Code` и копируем путь на файл .git.
>В терминале вводим: **git clone путь_к_файлу.git**

В нутри Вашей папки появится папка Fork`нутого проекта.
>С помощью команды: **cd папка_проекта** переходим внутрь.
### **Создание ветки для внесения своих изменений**
Проверяем **git branche** что бы понять в какой ветке мы находимся.\
При необходимости создаем свою личную ветку для внесения измененений, возможности отката и пр. безопасности.
### Работа над проектом
Пишем код, текст, рисуем картинки или то, что требуется по проекту.
>git add, git commit ...

вобщем работаем!!!
### Обновление FORK`a
По завершении работы нужно внести изменения в FORK`нутый проект.\
Делается это командой: **git push --set-upstream _origin branch_name_**\
Где **branch_name** это имя вашей основной ветки которую Вы создавали вначале работы.
### Отправка запроса на внесение изменений
Возвращаемся на GitHub в свой репозитарий в проект, над которым работали.\
Жмем зеленую кнопку `Compare & pull request`.\
При необходимости вносим коментарии и подтверждаем отправку изменений в основной проект.
## Заключение
